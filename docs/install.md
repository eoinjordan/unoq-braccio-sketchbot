...
| --- | --- |
| `Sketchbot-Tablet-0.3.2.apk` | Android 7+ controller WebView, device address entry, system camera/photo picker and paired-gamepad forwarding |
| `Sketchbot-Launcher-0.3.2.msi` | Windows per-user Start Menu/browser launcher; not a motor driver or a native camera server |
| `Sketchbot-Source-0.3.2.tar.gz` | Source, installer and prebuilt web assets; Python dependencies still download during setup |
